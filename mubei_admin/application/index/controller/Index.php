...
class Index extends Common
{
    ...
    public function select_user_p1(){
        ...
        if($order){
            $str='';
            foreach($order as $v){
                if($v['f_uid'] == 0){ $fuid= '初级会员'; }else{ $fuid=$v['f_uid'];}
                if($v['era'] == 0){ $agent= '初'; }else{ $agent='第'.$v['era'];}
                if($v['vip_static'] == 1){ $vip= '是'; }else{ $vip= '否';}
                if($v['best_uid'] == 0){ $best= '初级会员'; }else{ echo $best=$v['best_uid'];}
                $delete = $v['u_id'];
