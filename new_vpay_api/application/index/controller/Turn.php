...
//        $u_id = Request::instance()->param('user');
        ...
        if($type && $pass_p && $user && $money && $phone){
            ...
            if($type == 1){  //余额互转
                $check=Db::table('mb_user')->where('u_id',$user)->field('assets,balance,tel,pay_pass,u_id,vip_static,f_uid,era')->find();
                $check_t=Db::table('mb_user')->where('tel',$phone)->field('assets,balance,tel,pay_pass,u_id,vip_static')->find();
                $u_id=$check_t['u_id'];
                if(md5($pass_p) == $check['pay_pass']){
    ...
    public function inc_speed($u_id,$money,$check,$level){
        $c_uid=Db::table('mb_user')->where('u_id',$u_id)->field('era,vip_static,f_uid,u_id,best_uid,assets')->find();
