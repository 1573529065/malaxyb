...
class Turnin extends Controller
{
    public function turn_in(){
        ...
        if($user){
            ...
            if($url2){
                ...
                return jsonp(['code' => 1, 'msg' => 'succeed','url'=>"http://qr.topscan.com/api.php?bg=f3f3f3&fg=ff0000&gc=222222&el=l&w=200&m=10&text=http://www.malaxyb.com/mubei/TurnOut/turn_out.html?u_id=$user"]);
            }else{
    ...
    public function turnin_recode(){
        $user = Request::instance()->param('u_id');
        if($user){
            $card  =Db::table('mb_balance_order')->where('u_id',$user)->where('type',7)->field('bo_money,bo_time,target_uid')->order('bo_time desc')->select();
