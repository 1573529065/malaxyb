<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:84:"E:\phpStudy\PHPTutorial\WWW\mall\public/../application/index\view\seller\search.html";i:1527570370;s:84:"E:\phpStudy\PHPTutorial\WWW\mall\public/../application/index\view\common\common.html";i:1527572840;}*/ ?>
<!DOCTYPE html>
            ...
            <div id="dashboard">
                ...
                <a href="javascript:add_seller();" style="float: left" class="btn blue"><i class="icon-plus">添加商家</i></a>
                 <form action="<?php echo url('index/Seller/search'); ?>" method='post' style="float:left;margin:0 5px">
                     ...
                     <input type="text" placeholder="商家用户名/手机号码/店铺名/分类名"name='search' value='<?php echo $search; ?>'>
                    
                <span>状态:</span>
                <select name="status" id="status" >
                    <option value="-1" <?php if($status == -1): ?>selected<?php endif; ?> >所有状态</option>
                    <option value="0" <?php if($status == 0): ?>selected<?php endif; ?>>已禁用</option>
                    <option value="1" <?php if($status == 1): ?>selected<?php endif; ?>>已启用</option>
                </select>
                 开始时间:
    ...
    function exportExcel(){
        ...
        if($('#order').children().length <= 0){
            ...
            if($('input[name="search"]').val() == ''){
                window.location.href = '/public/index/Seller/sellerExport/status/'+$('select[name="status"]').val()+'/time1/'+$('#test1').val()+'/time2/'+$('#test2').val();
            
            }else{
                 window.location.href = '/public/index/Seller/sellerExport/search/'+$('input[name="search"]').val()+'/status/'+$('select[name="status"]').val()+'/time1/'+$('#test1').val()+'/time2/'+$('#test2').val();
            }
        }
