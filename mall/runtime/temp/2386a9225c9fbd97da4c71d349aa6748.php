<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:83:"E:\phpStudy\PHPTutorial\WWW\mall\public/../application/index\view\goods\search.html";i:1527563625;s:84:"E:\phpStudy\PHPTutorial\WWW\mall\public/../application/index\view\common\common.html";i:1527572840;}*/ ?>
<!DOCTYPE html>
            ...
            <div id="dashboard">
                ...
                <a href="javascript:add_goods();" style="float: left" class="btn blue"><i class="icon-plus"></i>添加商品</a>
                 <form action="<?php echo url('index/Goods/search'); ?>" method='post' style="float:left;margin:0 5px">
                    ...
                    <?php if($admin['type'] == 1): ?>
                     <input type="text" placeholder="输入商品名/分类名/店铺名"name='search' value='<?php echo $search; ?>'>
                    <?php else: ?>
                     <input type="text" placeholder="请输入商品名"name='search' value='<?php echo $search; ?>'>
                    <?php endif; ?>
                <span>上架状态:</span>
                <select name="status" id="status" >
                    <option value="-1" <?php if($status == -1): ?>selected<?php endif; ?> >所有状态</option>
                    ...
                    <div class="portlet-body no-more-tables" style="padding-bottom: 5%;">
                        ...
                        <table class="table-bordered table-striped table-condensed cf">
                            ...
                            <tr>
                                ...
                                <?php endif; ?>
                                <th class="numeric"><img src="<?php echo $data['goods_img']; ?>" alt="" style="height:100px"></th>
    ...
    function exportExcel(){
        ...
        if($('#order').children().length <= 0){
            ...
            if($('input[name="search"]').val() == ''){
                window.location.href = '/public/index/Goods/goodsExport/type/'+<?php echo $admin['type']; ?>+'/status/'+$('select[name="status"]').val()+'/time1/'+$('#test1').val()+'/time2/'+$('#test2').val();
            
            }else{
                 window.location.href = '/public/index/Goods/goodsExport/type/'+<?php echo $admin['type']; ?>+'/search/'+$('input[name="search"]').val()+'/status/'+$('select[name="status"]').val()+'/time1/'+$('#test1').val()+'/time2/'+$('#test2').val();
            }
        }
